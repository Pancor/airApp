...
public class Smog extends Renderable {
    ...
    public Smog(Bitmap bitmap, Bitmap foam, float y, float height, float width, int waves) {
        super(foam, 0, y);
        this.height = height;
        this.width = width;
        numberOfWaves = waves;
        ...
        smog = new PathBitmapMesh(VERTS, 1, bitmap, 1500);
    ...
    public void update(float deltaTime, float wind) {
        ...
        smog.matchVertsToPath(waterPath, height, ((bitmap.getWidth() / numberOfWaves) * 4f));
        for (Foam foam : foams) {
